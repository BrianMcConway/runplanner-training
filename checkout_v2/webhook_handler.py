...
class StripeWH_Handler:
    ...
    def handle_checkout_session_completed(self, event):
        ...
        # Call payment_intent.succeeded handler
    ...
    def handle_payment_intent_payment_failed(self, event):
        ...
        """
        logger.warning(
            f'Payment failed for PaymentIntent {event["data"]["object"]["id"]}'
        )
        return HttpResponse(
    ...
    def handle_checkout_session_expired(self, event):
        ...
        """
        logger.info(
            f'Checkout session expired for Session ID '
            f'{event["data"]["object"]["id"]}'
        )
        return HttpResponse(
            content=(
                f'Webhook received: {event["type"]} | '
                f'Handled checkout.session.expired'
            ),
            status=200
        )
    ...
    def handle_charge_succeeded(self, event):
        ...
        try:
            ...
            order.save()
            logger.info(
                f'Order {order_id} marked as paid via charge.succeeded.'
            )
            return HttpResponse(
                content=(
                    f'Webhook received: {event["type"]} | '
    ...
    def handle_charge_updated(self, event):
        ...
        try:
            ...
            return HttpResponse(
                content=(
                    f'Webhook received: {event["type"]} | '
                    f'SUCCESS: Charge updated.'
                ),
                status=200
            )
        except Order.DoesNotExist:
            logger.error(
                f'Order {order_id} not found for charge update.'
            )
            return HttpResponse(
                content=(
                    f'Webhook received: {event["type"]} | '
