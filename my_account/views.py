...
def purchased_plans(request):
    ...
    return render(
        request,
        'my_account/purchased_plans.html',
        {'purchases': purchases}
    )
...
def view_training_plan(request, id):
    ...
    training_plan = get_object_or_404(
        Product,
        id=purchase.training_plan.id
    )

    return render(
        request,
        'my_account/training_plan_details.html',
...
def delete_account(request):
    """
    Delete Account Page:
    Prompts the user to confirm account deletion and handles the process.
    """
    if request.method == 'POST':
